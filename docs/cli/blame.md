...
```
wipnote blame <path> [--format text|json|markdown] [--since YYYY-MM-DD] [--top N]
```

## Description

`wipnote blame` queries the `feature_files` attribution index and returns every
...
This command is the foundational primitive for file-to-track lineage. The `wipnote
...
find internal/db -type f -name '*.go' | \
  xargs -n1 wipnote blame --format json | \
...
git ls-files '*.go' | \
  xargs -I{} sh -c 'wipnote blame --format json {} 2>/dev/null | jq -r "if .features | length > 0 then .path else empty end"' | \
...
Run `wipnote reindex` to rebuild the attribution index if the list is unexpectedly large.
