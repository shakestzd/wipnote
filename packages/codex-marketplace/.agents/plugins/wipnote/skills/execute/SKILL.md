...
Use the title (not the raw track ID) as the outer `call spawn_agent with the appropriate agent_type` description when spawning the top-level coordinator:

```
call spawn_agent with the appropriate agent_type  # ✓ GOOD
```

Never use the raw track ID:
```
call spawn_agent with the appropriate agent_type  # ✗ BAD
```
...
Call spawn_agent with agent_type "wipnote-feature-coder" and message containing: description="feat-001: Add check command"; isolation="worktree"; prompt="[full task spec — see template below]".

Call spawn_agent with:
Call spawn_agent with agent_type "wipnote-feature-coder" and message containing: description="feat-002: Add budget command"; isolation="worktree"; prompt="[full task spec]".
...
while True:
    ...
    for task in no_overlap:
        ...
        Call spawn_agent with:
        Call spawn_agent with: description=task.subject,  # Must be slice.title from TaskCreate, NOT slice.num or feature_id; agent_type=task.metadata.agent; isolation="worktree"; prompt=build_prompt(task).
